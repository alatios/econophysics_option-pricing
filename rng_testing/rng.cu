...
// CURAND ADAPTER
//
__device__ __host__ RNG_CurandAdapter::RNG_CurandAdapter(){
	curand_init(clock64(), 0, 0, &(this->_State));
}

__device__ __host__ RNG_CurandAdapter::RNG_CurandAdapter(const unsigned int seed, const unsigned int sequence){
	curand_init(seed, sequence, 0, &(this->_State));
}

// Virtual functions from base class
__device__ __host__ void RNG_CurandAdapter::ResetSeed(){
	curand_init(clock64(), 0, 0, &(this->_State));
}

__device__ __host__ double RNG_CurandAdapter::GetUnsignedInt(){
	return this->GetUniform() * UINT_MAX;
}

__device__ __host__ double RNG_CurandAdapter::GetUniform(){
	return curand_uniform(&(this->_State));
}
__device__ __host__ double RNG_CurandAdapter::GetGauss(){
...
__device__ __host__ void RNG_CurandAdapter::SetInternalState(RNG* supportGenerator){
	curand_init(supportGenerator->GetUnsignedInt(), 0, 0, &(this->_State));
}
